       ...
       if (!user) {
         user = await prisma.user.create({
            data: {
                ...
                role: "partner", // This relies on a 'role' field which might be missing in User model based on schema? 
                // Wait, schema check: User model has 'role' field? 
                // Let's check schema.prisma lines 15-47. It has no 'role' field, it says "Derived from relation".
